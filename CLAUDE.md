...
**Macro-Tactics: The Workbench Defense** â€” a game jam entry on the theme
...
1. **Preparation Phase â€” creating Order.** No enemies, battle clock not
   running. The player freely drags, drops and rotates physical scraps (EVA
   foam, 3D-printing rejects, rulers) anywhere on the bench â€” **no grid, no
   snapping, no placement rules** â€” to build a maze and form choke points.
2. **Action Phase â€” surviving Disorder.** The player locks the layout in with a
...
### The "Order in Disorder" twist (enemy AI â€” NOT YET IMPLEMENTED)
...
line-of-sight to the player abandons its rule entirely and rushes them â€” so a
badly exposed firing position turns the player's structured maze back into
chaos.

Underneath sits a Source-engine (Counter-Strike) style movement controller â€”
ground/air acceleration, bunny-hopping, air-strafing, crouch â€” and a two-weapon
combat kit (hitscan pistol, melee sword).

Config: `project.godot`. Autoloads: `GameSettings` (`scenes/GameSettings.gd`) â€”
persisted sensitivity/volume and Difficulty state (`enum Difficulty { EASY, MEDIUM, HARD }`); `GameState` (`scenes/GameState.gd`) â€” the round
phase machine.

### Scene entry points

**`run/main_scene` is `res://scenes/main_menu.tscn`** â€” the game boots into the
...
**`scenes/main_menu.tscn`** (script `scenes/Control.gd`) â€” Start â†’ hides `MainButtons`
and shows `DifficultyPanel` (`EasyButton`, `MediumButton`, `HardButton`,
`BackButton`); Setting â†’ hides `MainButtons` and shows the `Setting` panel in
place (no scene change), with Exit Setting reversing it; Credits â†’
`res://scenes/credit_scene.tscn`; Quit â†’ `get_tree().quit()`.

**The run-in is Start â†’ DifficultyPanel â†’ How to Play â†’ `Main.tscn`.** Three
panels share the screen and only one is ever visible.
- `_on_easy/medium/hard_pressed()` all funnel through `_select_difficulty()`,
  which **records `GameSettings.difficulty` and advances to the How to Play card â€”
...
- `_on_how_to_play_start_pressed()` â†’ `_start_game()` is the **only** route into
...
- **`How to PLay/Button5` is connected to the menu script**, not to itself. It
  ...
  `change_scene_to_file()` directly and **never set `prologue_requested`** â€” so
  the kitchen prologue silently never ran. `Next_HTP.gd` is still attached to the
  button, but nothing is connected to it now.
- **The Start and Setting signals were crossed in the .tscn** â€” `MainButtons/Start`
  was connected to `_on_setting_pressed` and vice versa â€” and `Control.gd`'s
...
- **`credit_scene.tscn` has no way back.** `credit_text.gd/finish_credits()` only
  ...
  right behaviour â€” auto-return when the scroll ends, or return on a key press â€”
  is a design call.
...
from the menu â€” `Control._start_game()` sets `GameState.prologue_requested`, and
`Player._ready()` consumes it. Running `Main.tscn` directly (F6) skips straight to
the bench, which is what every test wants.

**The player is scaled to `PROLOGUE_SCALE` (74.76889) â€” the same factor `Kitchen`
and `meja` carry â€” and that is the point.** Measured: the book is 26 units wide
and floats 72 units above the walkable floor. At 1Ã— that is an unreachable
26-metre monolith; at 74.77Ã— it is **0.35 m at waist height**, i.e. a real book on
a real counter. The curse then restores 1Ã—, which is what makes the bench
enormous. `_base_movement` captures `max_speed`, `walk_speed`, `crouch_speed`,
`jump_velocity` and `gravity` and scales them by the same factor â€” leaving them
at 1Ã— would have a 134-unit-tall figure crawling across the room â€” then restores
them on the shrink.
...
- **`is_prologue` suppresses the fall-death check.** The kitchen floor is at y = 0
  ...
  `_physics_process` â€” returning there would freeze the player mid-prologue.
...
- **HARD's pitch-black is deferred until the curse.**
  ...
  **whatever the difficulty**, and only blacks the world out once the prologue has
  ...
  - **Both branches set every property the other touches.** The `Environment` is
    ...
    its state leaves the rest behind â€” in particular the daylight branch must put
...
- **Walking pace is `prologue_speed` (3.5), applied as
  `prologue_speed * PROLOGUE_SCALE`.** It overrides crouch and walk alike â€” the
  prologue is one deliberate stroll. The scaling is not optional: at a raw
  3.5 u/s a 74.77Ã—-sized player crosses the 763-unit room in **218 seconds**;
  scaled, it is 3.
- **The kitchen is solid.** `kitchen.tscn` carries a `Collisions` StaticBody3D
  (layer 1, mask 0) with **9 box shapes** in kitchen-local units â€” four walls at
  Â±5.4, a room floor whose top is y = 0, and a counter ring at Â±4.4. Without it
  ...
  74.77Ã— instance; uniform-scaled static boxes are fine, but this is the one
...
- **The prompt label lives in `Main.tscn` as `BookPromptLabel`**, not in
  `buku.tscn`. It is positioned in world space at (59.477, 92.2, âˆ’263.064) â€”
  directly over the book â€” which is far easier to reason about than the book's own
  local frame, where the mesh sits ~34 units up and 5 across from the node origin.
  Gold "â–¼ / [E] Open Book" in `Dimbo Regular.ttf` at `font_size` 32 /
...
- **`Player._update_book_prompt()` drives it**, from `_process`: visible only
  ...
  (0.003 rad/ms, Â±0.6 units) â€” clock-driven so it cannot drift out of phase, and
  ...
  - **Both labels use `fixed_size = false` with `pixel_size` 0.0015, `font_size`
    ...
    constant *screen* size and `pixel_size` becomes a screen scale â€” at 0.014 with
    ...
    tall** (~20 px at 25 units on a 1080p 70Â° view) and the prompt 0.48. That is
    readable but modest â€” raise `pixel_size` toward 0.003 if the marker needs to
  ...
  - Both labels are positioned at the book's **measured** local offset
    (âˆ’3.235, y, 3.88), not at the node origin â€” the mesh sits well off it. It reads `interact` in **`_input`, not `_unhandled_input`**, and
  consumes the press â€” `Player._unhandled_input()` binds the same key to prop
...
- **`HUD/SubtitleLabel` is set in `Dimbo Regular.ttf`**, the menu's face, at 36 pt
  in the same gold (1, 0.85, 0.2) as the book marker with a 12 px black outline â€”
  ...
  `autowrap_mode` 2; measured, the curse text needs 185 px of height in an 872 Ã—
...
- On inspection the player freezes, `HUD/BlackOverlay` fades to black over
  `FADE_TIME` (0.9 s) and `HUD/SubtitleLabel` shows the curse â€” **5 coins on EASY,
  10 otherwise**, matching `CoinSpawner.coin_count`. Space (`ui_accept`) fades
  back out, restores 1Ã— scale and the movement tunables, teleports to the
...
- `start_action()` (F) locks the layout in; `reset_round()` (G, and later death
  ...
  **Enter** (`toggle_action_phase`) flips whichever way the round currently is â€”
...
- Resetting deliberately does **not** move the obstacles â€” keeping the layout
...
### `scenes/Main.tscn`
- `WorldEnvironment` â€” procedural sky, tonemap.
- `DirectionalLight3D` â€” shadows enabled.
- `Floor` (CSGBox3D, ~110Ã—1Ã—50) + `Ramp` + `Stair0`â€“`5` â€” leftover movement-test
  geometry on the ground, well away from the bench.
- `meja` â€” instance of `meja.tscn` scaled ~74.8Ã—, **the workbench itself**. It
  ...
  around and gets shoved by them. **Tabletop surface sits at y â‰ˆ 73.635**, and
  the scale carries a slight tilt, so the surface drifts ~0.15 across the bench.
- `Kitchen` â€” instance of `res://kitchen.tscn` (note: project **root**, not
  `scenes/`), the room the workbench stands in. Placed at
  `(50.5054, 0, -4.5812955)` with a clean axis-aligned scale of **74.76889 â€”
  ...
  around a 74.8Ã— table would look like a dollhouse instead of a real room. At
  this scale the walls sit Â±389 units out around a bench roughly 112 units in
  ...
  - The `y` is 0 and not an offset because `kitchen.tscn`'s floor CSGBox already
    has its top face at local y = 0 (centre âˆ’0.1, `size.y` **1.0**, scale 0.2).
    `CSGBox3D.size` defaults to 1, not 2 â€” assuming 2 puts the whole room 7.5
  ...
  - **Three nodes in `kitchen.tscn` are hidden (`visible = false`), and all three
    ...
    - `CSGBox3D` and `CSGBox3D2` â€” the room's floor and ceiling slabs. Untextured
      CSG boxes 10.2 units square, which at 74.8Ã— become **763-unit teal slabs**
      that swamp the view from the bench. The floor also sits at y â‰¤ 0 where
      `meja`'s legs reach down to y = âˆ’2, so it cut through the bottom of the
      table.
    - `KitchentableBLarge` â€” the kitchen ships its own `kitchentable_B_large` at
      ...
      volume**, y âˆ’0.28 â€¦ 74.77 across the whole bench: two different tables
    ...
    - These are hidden **in `kitchen.tscn` itself, not by an instance override in
      `Main.tscn`.** An override was tried first and did not survive â€” opening
      `Main.tscn` in the editor and saving dropped the hand-written
      `[node name="â€¦" parent="Kitchen" index="4"]` block silently, and the table
  ...
  - **Consequence of hiding the floor slab:** the only ground left is `Main.tscn`'s
    own `Floor` CSGBox, which is 292 Ã— 224 at y = 0 â€” much smaller than the
  ...
  - **Do not re-run `tools/BulkPropSetup.gd` without adding `Kitchen` to
    ...
    room in a RigidBody3D full of hulls. (`rebuild_prop_colliders.gd` is safe â€”
    it only touches bodies already in the `draggable` group.)
- `PrepCamera` (Camera3D + `PrepCamera.gd`) with a `MouseRay` (RayCast3D) child
  â€” the Preparation Phase tactical view. See Â§2b.
- **~500 converted props** â€” the maze-building clutter (crates, furniture, food,
  ore, tunnel piecesâ€¦), all direct children of the root. Each is a
  ...
  converter in Â§2e. **They are frozen** (`FREEZE_MODE_KINEMATIC`) and their
  obstacle avoidance is off; `PrepCamera` thaws and un-mutes exactly the one
  being dragged. See Â§2e for the measurements behind both decisions.
- `bowl_dirty2` â€” the original drag-and-drop test object, deliberately left
  **fully dynamic** (unfrozen) as the reference the props were tuned against. A
  ...
  r 7.887 Ã— h 4.9815, offset +2.49 y). **The scale lives on the `Model` child,
  not on the body** â€” the mesh is 0.95 Ã— 0.3 Ã— 0.95 at 16.605Ã—, and putting that
...
- `Player` â€” instance of `Player.tscn`, above the bench.
- `Navigation` (NavigationRegion3D + `NavBaker.gd`) with its `NavSurface` child,
  `EnemyGoal` (Marker3D, group `enemy_goal`) and an `Enemy` instance â€” the
  pathfinding prototype. See Â§2d.
- `WaveSpawner` (Node3D + `WaveSpawner.gd`) â€” the timed wave system. See Â§2f.
- `CoinSpawner` (Node3D + `CoinSpawner.gd`) â€” the collectible win condition. See Â§2g.
- `RoundUI` (CanvasLayer) â†’ `CoinLabel` â€” top-right "COINS n/10" readout, hidden
  outside ACTION. Separate from the player's own HUD because it is a round-level
  concern, not a first-person one.

Obstacle scripting (`Obstacle.gd`, `BuildMode.gd`, Â§2c) still exists but has no
nodes in `Main.tscn` right now â€” the block layout was reverted while the drag
...
- Authored at **75Â° below horizontal**, 125.6 units above the tabletop, aimed
...
- Picking uses `project_ray_origin()`/`project_ray_normal()` from the cursor to
  ...
  â€” it is driven by hand, not by its own -Z). `target_position` is local, so the
  world direction is converted with `to_local()` â€” this node is pitched 75Â°.
- Hold LMB to drag, release to drop; the wheel spins the held object.
- **The drag is entirely physics-based â€” nothing writes `global_position` or
  ...
  makes it lean and swing on its own â€” the engine derives the tilt, not the
  ...
  - `tilt_arm` clamps the lever arm. At this world scale the raw grab offset is
    ...
    175Â° before clamping); 0.6 keeps the lean proportional and settleable.
  - `_upright_torque_for()` adds the restoring torque that turns a one-off lean
    ...
    flew 62 units further before landing â€” and yes, you can fling it clean off
    the bench).
- Tunables: `spring_stiffness` (120), `spring_damping` (14 â€” critical is ~22, so
  ...
  ~38Â° of lean and settles back toward level once the cursor stops.
...
- **A held prop also gets a collision exception with the PLAYER**, added in
  ...
  barge them off it â€” with the tabletop at y â‰ˆ 73.6 and `FALL_DEATH_Y` at 65,
...
- Draggables are found by **group** (`draggable`), not node path â€” tagging more
...
- **Grabbing thaws, releasing re-freezes.** Converted props sit frozen so 500 of
  ...
  the carry spring entirely) and `_drop()` does **not** freeze on the spot â€”
...
- **Grabbing also switches that prop's RVO avoidance on, and release switches it
  off** â€” see the cost measurement in Â§2e.
- **Fallen props are despawned.** `_clean_fallen_props()` runs from `_process`
  ...
  table â€” deliberate, and measured â€” and the player or an enemy can shove one
  ...
  - The threshold is safe by a wide margin, measured on the real scene: the
    tabletop is at y â‰ˆ 73.6, all 522 props sit between **72.37 and 79.02**, and
    the leftover ground geometry is at y = âˆ’0.5. The lowest prop clears the
  ...
  - Guarded with `is_queued_for_deletion()` â€” a freed node stays in its group
...
â”œâ”€â”€ CollisionShape3D / MeshInstance3D   (capsule; resized at runtime for crouch)
â”œâ”€â”€ Head (Node3D)                       -- pitch pivot, eye height
â”‚   â””â”€â”€ Camera3D                        -- yaw is on Player root, pitch on Head
â”‚       â”œâ”€â”€ RayCast3D                   -- shared aim ray, target 100m forward
â”‚       â”œâ”€â”€ Pistol  (instance of Pistol.tscn, Slot 1)
â”‚       â””â”€â”€ Sword   (instance of sword.tscn, Slot 2)
â”œâ”€â”€ BuildMode (Node3D, BuildMode.gd)  -- Preparation-phase obstacle dragging
â”œâ”€â”€ VoicePlayer (AudioStreamPlayer)   -- the three narration beats, one at a time
â”œâ”€â”€ MusicPlayer (AudioStreamPlayer)   -- looping BGM, volume_db -7
â”œâ”€â”€ SFXPlayer   (AudioStreamPlayer)   -- AudioStreamPolyphonic mixer, 16 voices
â”œâ”€â”€ jump (AudioStreamPlayer3D)
â””â”€â”€ HUD (CanvasLayer)
    â”œâ”€â”€ StatsLabel     -- velocity/pos/angle/bhop-gain debug readout; HIDDEN
    â”œâ”€â”€ PhaseLabel     -- phase/attempt/keys debug readout; HIDDEN
    â”œâ”€â”€ GunBar         -- ProgressBar, Stasis Cannon charge; hidden when sword equipped
    â”œâ”€â”€ PlayerHealthBar -- ProgressBar, 100 HP; hidden except during the boss duel
    â”œâ”€â”€ StaffBar       -- ProgressBar, Staff of Quartz heavy-attack charge
    â”œâ”€â”€ BossHealthBar  -- ProgressBar + "ARCHMAGE MALAKOR" label, hidden until the duel
    â”œâ”€â”€ Crosshair      -- drawn Control (Crosshair.gd), auto-hides when mouse not captured
    â”œâ”€â”€ Minimap        -- drawn Control (Minimap.gd), top-left tactical radar
    â”œâ”€â”€ PrepTutorialPanel -- build-phase instructions, PREPARATION only
    â””â”€â”€ YouDiedLabel   -- centred "YOU DIED" retry prompt, hidden unless dead
â””â”€â”€ SettingsMenu       (instance of SettingsMenu.tscn) -- Esc pause/settings overlay
```

(`SettingsMenu` is a child of the Player root, **not** of `HUD` â€” it brings its
own CanvasLayer.)

### Narration & the Ending

Three voice lines mark the three turns of the story, all played through one
`VoicePlayer` (`AudioStreamPlayer` on the Player) so a new line always cuts the
last one off instead of overlapping it. `_play_voice()` no-ops if the player or
the stream is missing.
- **`VO_SCENE_1`** â€” fires in `_on_book_inspected()`, under the curse blackout.
- **`VO_SCENE_2`** â€” fires in `begin_boss_fight()`, as the Archmage arrives.
- **`VO_SCENE_3`** â€” fires in `_on_boss_died()`, which is the ending.

Narration can be cut short as well as started. **`_stop_voice()` is called when
Space dismisses the curse blackout**, so `scene 1.mp3` ends with the screen it
belongs to. Without it a quick reader carries the curse being pronounced out onto
the bench, over the top of the track `play_tabletop_music()` starts a line later.

**The ending sequence (`_on_boss_died`)** freezes the player, **shuts the swarm
down**, holsters the staff, clears the combat HUD, plays the closing line, and
then **`await voice.finished`**
before fading `BlackOverlay` to solid and putting `VICTORY_TEXT` up in
`SubtitleLabel`. Awaiting the signal rather than a fixed delay means the fade
always lands on the last word whatever the recording's length.
- **Both `voice != null` and `voice.playing` are checked before the await.** An
  await on a signal that can never fire hangs forever, so a missing or unloadable
  audio file would soft-lock the ending on a black screen â€” the worst possible
  place for it.
- `_in_victory` is re-checked after each await: the player can reset the round
  while the line is still playing, and the sequence must not then paint an ending
  card over a live game.
- **`_end_the_swarm()` runs before the awaits, not after.** Winning must leave
  nothing hostile alive: a minion still on the bench would keep hunting a player
  who is frozen and unarmed behind the ending card, and its contact kill would put
  "YOU DIED" over the victory screen. It stops every `wave_spawner` first and only
  then sweeps the `enemies` group — freeing the minions before stopping the clock
  leaves a timer that can fire on a later frame and repopulate an arena the player
  has already won. Both lookups are by group (the spawner is in `Main.tscn`, the
  Player is an instance inside it), and the spawner is called through
  `has_method()`, so a scene with no spawner still clears its enemies.
  `WaveSpawner.stop_waves()` is the public half: stop the timer, rewind `_wave` to
  0, `clear_enemies()`. It is distinct from `_apply_phase()`'s PREPARATION branch,
  which does the same two things as a *reset* — this one means the round is over.
  Both are idempotent.
- While `_in_victory`, `_physics_process` freezes horizontal movement and
  `_unhandled_input` accepts **only** Space, which calls `_return_to_menu()`.
- **`_return_to_menu()` resets `GameState` before changing scene** â€” `boss_fight`,
  `prologue_active` and `phase`. The autoload outlives the scene change, so
  without this the menu would start the next run still believing a boss fight was
  in progress.

### Music & Sound Effects

All game audio lives on the Player, in **three** `AudioStreamPlayer`s with three
different jobs: `VoicePlayer` (narration, above), `MusicPlayer` (one looping BGM
track at a time) and `SFXPlayer` (everything else, all at once). Everything runs
on the Master bus, so `GameSettings.volume` already governs the lot.

**`MusicPlayer`** â€” `volume_db = -7.0`, so the track sits under the narration and
the effects rather than over them. Three tracks, all preloaded in `Player.gd`:

| Constant | File | When |
|---|---|---|
| `BGM_EZDIF` | `ezdif.mp3` | Tabletop play on EASY and MEDIUM |
| `BGM_HARD` | `hard.mp3` | Tabletop play on HARD |
| `BGM_BOSS` | `boss.mp3` | The Archmage duel |

- **`_play_music()` turns looping on, because the imports do not.** All three
  `.import` files carry `loop=false`, so without `mp3.loop = true` each track
  plays through once (`ezdif.mp3` is 108 s) and the game falls permanently silent.
  It is set on the stream resource at runtime rather than in the `.import` files,
  which the editor rewrites whenever it re-imports.
- **Re-requesting the track already playing is a no-op.** `play_tabletop_music()`
  is called from more than one place, and restarting a loop the player is already
  listening to drops an audible gap into it.
- **`play_tabletop_music()` is called at the point tabletop play actually begins,
  which is not `_ready()` on the normal route.** Coming from the menu the prologue
  runs first, so `_ready()` starts nothing and `_end_prologue()` starts the track
  as the curse lands â€” the kitchen plays under the narration alone. Running
  `Main.tscn` directly (F6) has no prologue to wait for, so `_ready()`'s `else`
  branch starts it immediately. Both routes are covered; neither starts it twice.
- **The two tracks are separated by silence, not crossfaded.**
  `begin_boss_fight()` calls `stop_music()` immediately before `_play_voice(VO_SCENE_2)`,
  so the bench track ends the instant the Archmage is announced and his
  introduction plays into nothing; `play_boss_music()` then brings `BGM_BOSS` in
  from `attach_boss()`, which runs once that line has finished. The narration is
  therefore the whole transition between the two tracks. That is also why
  `play_boss_music()` is **not** in `begin_boss_fight()` — it runs *while*
  `scene 2.mp3` plays, so starting the battle loop there would put it under the
  introduction. In `attach_boss()` it sits **above** the null-check on the health
  bar, since the music should change whether or not there is a bar to wire up.
- **`_on_round_reset()` returns to the bench track and `_on_boss_died()` stops the
  music.** Neither is in the original brief and both are needed: a retry out of the
  duel frees the boss, so leaving `BGM_BOSS` looping would rebuild the maze under
  battle music with no battle, and the ending's closing line and victory card
  should not play over a combat loop. `_return_to_menu()` needs nothing â€” the scene
  change frees the Player and its music with it.

**`SFXPlayer`** â€” `max_polyphony = 16`, and **its stream is an
`AudioStreamPolyphonic` (`polyphony = 16`) authored in `Player.tscn`**. That second
part is load-bearing and not optional:

- **`max_polyphony` alone does not do what it looks like it does.** Measured:
  `AudioStreamPlayer.set_stream()` calls `stop()`, so assigning a different sound
  kills whatever is playing. `max_polyphony` only ever overlaps repeats of the
  **same** stream. With one shared player and four different effects, a coin
  pickup would cut a cannon shot dead. Verified the other way too: an
  `AudioStreamPolyphonic` plays three distinct streams concurrently, all three
  voice ids valid and alive.
- `_setup_sfx()` (from `_ready()`) starts the mixer and caches its
  `AudioStreamPlaybackPolyphonic`. The mixer stream never ends, so the player stays
  `playing` for the whole session and the handle stays valid. It rebuilds the
  polyphonic stream if the scene lacks one, so a hand-stripped `Player.tscn` still
  makes noise.
- **`Player.play_sfx(stream)` is the single public entry point.** Nothing else owns
  an audio player.

| Constant | File | Fired by | Where |
|---|---|---|---|
| `SFX_PISTOL` | `pistol.mp3` | `Pistol.gd` | `_fire()` |
| `SFX_LIGHTNING` | `lightning.mp3` | `StaffQuartz.gd` | LMB bolt |
| `SFX_FIREBALL` | `fireball.mp3` | `StaffQuartz.gd` | RMB heavy orb |
| `SFX_COIN` | `pickupcoin.mp3` | `CoinSpawner.gd` | `_on_coin_collected()` |
| `SFX_FAH` | `fah.mp3` | `Player.gd` | `die()` â€” the death jumpscare |
| `SFX_EXPLOSION` | `explosion.mp3` | `BossMage.gd` | `SpellOrb._spawn_impact()` and `_on_landed()` |

- **Each script preloads only the sounds it plays**, rather than all seven
  everywhere. The three BGM constants live in `Player.gd`, which is the only thing
  that starts music.
- **Nobody holds a node path to the mixer.** `Pistol` and `StaffQuartz` walk up the
  tree for the first ancestor with a `play_sfx` method â€” the same shape as
  `StaffQuartz._aim_camera()` / `_shooter()`, so a weapon survives being
  re-parented or tested on its own. `CoinSpawner` is in `Main.tscn`, not under the
  Player, so it goes through its existing `player_group` export instead.
- **The coin sound has to be played by the spawner, not the coin.** `Coin.gd`
  `queue_free()`s itself on collection, so a player of its own would be freed
  mid-chime.
- The staff's primary is on a 0.18 s cooldown, i.e. about five bolts a second.
  Overlap is the expected case here, which is what the 16 voices are for â€” measured
  as surviving 20 overlapping bolts in a single frame.
- **The death sting rides `die()`'s existing idempotence.** `die()` early-returns
  when `is_dead` is already set, so a whole swarm arriving on one frame gets one
  `fah.mp3` rather than a wall of them â€” no separate guard needed.
- **The Archmage's explosion is placed at the two impact sites, deliberately NOT
  inside `_shake_player()`.** That helper is shared with `_cast_spell()`, which
  shakes the camera as the orb *leaves* â€” routing the sound through it would
  detonate every spell at the moment it was fired. The two real sites are
  `SpellOrb._spawn_impact()`, which is the one call every impact reaches (direct
  hit, splash and a dud against scenery alike, so it can neither double-fire nor
  miss a case), and `_on_landed()` for the crushing leap.
- **`SpellOrb` finds the mixer by GROUP**, the same `player_group` handle
  `_splash()` uses: the orb is parented to the scene root, so there is no path to
  the player, and it frees itself on the frame it hits â€” an audio player of its own
  would be cut off mid-sound. `BossMage._play_sfx()` re-resolves the player rather
  than caching it, since a leap can land after a reset has replaced them.
- **Everything is non-positional** (`AudioStreamPlayer`, not `AudioStreamPlayer3D`),
  so an orb detonating against distant scenery is as loud as one at the player's
  feet. Tolerable because the boss holds `preferred_distance` (12), which keeps
  impacts near the player anyway â€” but it is why an explosion may read as closer
  than it looked.
...
- **Falling off the bench kills.** `_physics_process` calls `die()` as soon as the
  player is below `FALL_DEATH_Y` (65.0) â€” the same threshold `PrepCamera` and
  `Enemy` use to despawn fallen props and enemies. The tabletop is at y â‰ˆ 73.6 and
...
- **`take_damage(amount)` spends `health` (100).** Only the Archmage's spells use
  it â€” a minion that reaches the player still calls `die()` outright, because
...
- **Gated on CHASE, not proximity.** A patroller brushing past on its way
  ...
  hidden-rule routes lethal to *stand near* rather than lethal to be *seen by* â€”
...
- **`Player.die()`** is idempotent (`if is_dead: return`), so a whole swarm
  ...
  sets `_shake_intensity` to 0.8, plays `SFX_FAH` (the death jumpscare â€” the same
  guard keeps it to one sting), shows `HUD/YouDiedLabel`, and drops any carried
  prop â€” a prop still on the carry spring would otherwise keep being steered by a
  corpse.
- **While dead**, `_physics_process` zeroes horizontal velocity and returns early
  ...
  reload or grab props. **Mouse look deliberately still works** â€” that block sits
...
- **`round_reset` is connected as well as `_respawn()` calling it.** Anything
  that resets the round â€” G straight through GameState, a future breached-defence
  trigger â€” must clear the death screen, or the player ends up alive and mobile
...
**`scenes/Minimap.gd`** â€” the top-left radar, a 160 Ã— 160 `Control` at
...
- **It exists because of the aggro mechanic.** Enemies only chase what they can
  ...
  step out from cover and look â€” which is precisely the mistake that gets the
...
- Contacts are found by group â€” `enemy_group` ("enemies", matching
  `WaveSpawner`), `prop_group` ("draggable"), `player_group` ("player") â€” so
...
- **Prop footprints are measured once and cached** by instance id. Only the
  ...
  visual bounds, then to a 2 Ã— 2 m block.
- **`_shape_bounds()` reads shape parameters directly â€” never
  ...
  multi-convex rebuild (Â§2e) props carry up to 8, so the first frame a prop
  entered radar range paid for up to 8 mesh builds. Measured over all 522 props:
  **103 ms â†’ 5.1 ms, a 20Ã— speedup**, with the footprints agreeing to a worst
  ...
  round, not just at load â€” which is what made the radar feel like an FPS drop.
...
- Only **7 of 522** props sit inside the 30 m radar radius on the current bench,
  so the per-frame drawing is trivial â€” the cost was always the full-group scan
...
**`HUD/PrepTutorialPanel` teaches the build phase.** A top-centre `PanelContainer`
over a translucent rounded box, holding two Labels in `Dimbo Regular.ttf` at
26 pt with an 8 px black outline: `PrepTutorialHeader` ("ðŸ› ï¸ MAZE BUILDING PHASE
ðŸ› ï¸") in gold and `PrepTutorialLabel` (the four bullets) in white.
- **Two Labels, not one.** A `Label` renders in a single colour, so the specified
  yellow header with white bullets is impossible in one node â€” the alternative
  was a `RichTextLabel` with BBCode. Two stacked Labels keep the plain-Label
  simplicity and the requested name.
- **Visibility is driven from `_process`, via `_update_prep_tutorial()`**, not
  from the handful of places the phase changes. It is up only when
  `GameState.is_preparation()` and not the prologue, the boss fight, the ending
  card or death â€” five conditions across four subsystems, and one missed call
  site would leave a tutorial panel sitting over the Archmage duel.
- `mouse_filter = 2` (IGNORE) on the panel and both labels. Load-bearing: it sits
  over the top of the screen during PREPARATION, which is exactly when LMB is the
  prop-drag tool, and a Control that accepts input there would swallow those
  clicks. Same trap as the Minimap.

**`StatsLabel` and `PhaseLabel` are debug scaffolding and ship hidden.** They read
as leftover developer text over the game â€” "ACTION - attempt 1 / G restart" and a
...
`ground_accel`, `friction`, `stop_speed`, `air_accel`, `air_cap` (0.8 â€” the
...
**Prop tunables** (`Player.gd`, `@export_group("Props")`): `push_force` (120 â€”
see the friction gotcha in Â§5 before lowering it), `carry_range` (3.0),
...
returns âˆ’1 during the prologue (a full-size person walking a kitchen carries
nothing), `STAFF_SLOT` during the Archmage duel (the staff is the curse-breaker
handed over for the fight), and slot 0 â€” the Stasis Cannon â€” for everything else.
...
belongs to the prop-drag tool** then â€” an armed cannon would fire every time the
...
**Slot 1 â€” Stasis Cannon** (`scenes/Pistol.tscn`, script `Pistol.gd` â€” node and
...
- **One shot every `cooldown_time` (25 s), and it slows rather than kills.** The
  ...
  Firing at the wrong moment leaves the player with nothing for 25 s â€” which is
...
- On a hit, `_apply_impact` offers the collider `apply_slow(slow_factor,
  slow_duration)` (0.4 / 5.0), then `take_damage` and `apply_hit` â€” **all three
...
- `cooldown_bar` (exported `ProgressBar` ref â†’ HUD/GunBar) shows charge as
  `(1 - timer/cooldown_time) * 100` â€” a meter **filling back up**, which reads as
  "ready?" far better than a countdown draining away. `_ready()` forces the bar's
  range to 0â€“100 rather than trusting the scene, since the value is a percentage.
...
- **Important:** the `Pistol` node in `Player.tscn` must declare
  ...
  these exported NodePaths silently resolve to `null` (see Â§5 below).
- Ammo, magazines and the reload animation were **removed** in the overhaul â€” a
...
**Slot 2 â€” Staff of Quartz** (`scenes/StaffQuartz.tscn`, script `StaffQuartz.gd`)
â€” the boss-fight weapon, built around `staff_A.obj`.
...
- **LMB** (`fire`) â€” a fast bolt, `bolt_damage` **15** on a `bolt_cooldown` of
  0.18 s. The chip damage. **RMB** (`fire_secondary`) â€” a heavy orb,
  `heavy_damage` **40** on a `heavy_cooldown` of **3.0 s**. The commitment.
  That is ~33 primary hits or ~13 heavy casts to take the boss's 1000 HP. The
  first pass shipped 5/25, which needed 100 clean hits â€” a slog, not a fight.
- **First-person pose** is set on the instance in `Player.tscn`, not in
  `StaffQuartz.tscn`: position (0.35, âˆ’0.35, âˆ’0.6), `rotation_degrees`
  (âˆ’12, 18, 0) relative to `Head/Camera3D` â€” held low and right, angled across
  the view. Note the 12-float `Transform3D` in the .tscn stores basis **rows**
  (see Â§5); writing the columns yields the transpose, which for a rotation is its
  inverse and reads back as roughly negated angles.
- `cooldown_bar` (â†’ `HUD/StaffBar`) shows the **heavy** charge; the primary's
  0.18 s is too short to read as a meter.
- Shots leave along the **camera's** forward, not the staff's own âˆ’Z: the staff is
...
- Damage lands through `has_method("take_damage")`, so the staff knows nothing
  ...
  `Bolt extends Area3D` class â€” layer 0 / mask 1, so they detect the world and
  nothing ever tests against them â€” with a `projectile_lifetime` (4 s) so a
...
- **Visuals come from `Spell Effects (Free)`**, carried as a child of the bolt:
  ...
  `spell_2_orb.tscn` and `spell_2_impact.tscn`. See Â§2i for the rules these rigs
  impose.

**Slot 2 (legacy) â€” Sword** (`scenes/sword.tscn`, script `Sword.gd`, wraps a `.blend`
katana model in `3DModels/`)
- Melee: reuses the **camera's** RayCast3D (not a locally-aimed one â€” the
  Sword node is rotated ~84Â° on Y in the scene, so its own -Z does not point
...
- No baked animation â€” slash is procedural: a `Tween` swings `rotation`/
  ...
  travel across the screen â€” small offsets read as an in-place twist, not a
...
**`Main.tscn/Navigation`** â€” `NavigationRegion3D` running `scenes/NavBaker.gd`.
- Bake source is **`NavSurface`**, a `StaticBody3D` (collision layer 512,
  mask 0) with a 212 Ã— 2 Ã— 138 box whose top face sits exactly on the tabletop.
  A dedicated layer nothing else masks means it never collides with anything â€”
...
- `source_geometry_mode = GROUPS_WITH_CHILDREN` on group `navmesh_source`.
  **Not `GROUPS_EXPLICIT`** â€” explicit mode visits only the exact grouped nodes
...
- `NavBaker.gd` watches every body in the `draggable` group and re-bakes once a
  ...
  `min_interval` (0.5 s). Debounced deliberately â€” a bake is far too expensive
...
**`bowl_dirty2/NavigationObstacle3D`** â€” carves the bowl out of the navmesh.
...
**`scenes/Enemy.tscn` / `Enemy.gd`** â€” `CharacterBody3D` (capsule collider r 0.4,
h 1.8) with a `NavigationAgent3D` (avoidance on, agent radius 0.5) and a
`Model` â€” an instance of KayKit's **`Skeleton_Minion.glb`**. The red prototype
...
- `Model` is scaled **0.831**, which takes the glb's native 2.166-unit height to
  exactly the capsule's 1.8 with feet at local y = 0 â€” verified as spanning
  0.000 â€¦ 1.800 against a capsule spanning 0.000 â€¦ 1.800.
- **`Model` is also rotated `(0, 180, 0)`**, because the skeleton is authored
  facing **+Z** while everything else here treats **âˆ’Z** as forward (`_drive()`
  aims yaw with `atan2(-x, -z)`, matching the player capsule). Without it the
  skeleton moonwalks â€” it runs backwards along its own path. Note this means the
  model's *local* âˆ’Z now points along the body's +Z, so **do not sanity-check the
  facing by comparing local axes**; it looks inverted and is correct. The
  reliable check is the geometry: the `Eyes` mesh sits at local z âˆ’0.216, ahead of
  the `Cloak` at âˆ’0.012, i.e. the face leads along the body's âˆ’Z forward.
...
- The `AnimationPlayer` is added by `Enemy.tscn` as a child of `Model`, so its
  default `root_node` of `".."` resolves to the model root â€” the same layout the
...
- It also forces `loop_mode = LOOP_LINEAR` on the run and idle clips. **The packs
  import one-shot** â€” without this the skeleton runs for 0.8 s then freezes
...
- Measured: 35 animated skeletons cost 5.61 ms/frame of physics against 5.83 for
  the old capsules â€” no regression. **Caveat: an `AnimationPlayer` updates on the
...
- `target_desired_distance` is **driven from the script per state**, not left at
  the authored value â€” see `_apply_arrive_tuning()` and the arrival gotcha in Â§5.
  Scene default 0.5; `path_desired_distance` 0.8.
- **Patrols a looping circuit** â€” every Node3D in the `enemy_goal` group is a
  ...
  reference â€” see the `node_paths` gotcha in Â§5.
...
- A stop counts as reached within `arrive_distance` (4.0), compared on the
  horizontal plane only â€” the markers sit on the tabletop while the capsule's
...
- **Two faster give-up paths, because 20 s of standing still reads in-game as the
  ...
  further than `arrive_distance` away â€” the path exists but stops short â€” or
  ...
  Measured on the sealed bench: longest freeze 19.7 s â†’ 1.5 s, ground covered in
  45 s 50 â†’ 140 units.
- With avoidance on, the desired velocity goes to `agent.velocity` and the move
  ...
  physics frame.
- This is a pathfinding/aggro testbed, not the shipping enemy â€” the patrol leg
...
**Aggro â€” the "Order in Disorder" twist.** Two conditions, both required:
- `DetectionArea` (Area3D, sphere **r 250**, layer 0 / mask 1) reports the player
  â€” found by the `player` group, which `Player.tscn`'s root now carries. 250 is
  **bench-wide**: the table is ~220 units end to end, so proximity is no longer a
  real gate and line of sight is doing all the work. That is the intent â€” clutter
...
- `SightRay` (RayCast3D at `eye_height` 1.5) reaches the player's chest
  unblocked. Anything else the ray hits first â€” the bowl, a wall â€” denies aggro.
...
is still "inside" for one frame â€” and since the ray is aimed at them *wherever
...
(1.25Ã—). Losing sight does not drop it instantly â€” `aggro_memory` (2 s) keeps
...
whenever a chasing enemy is further than `chase_arrive_distance` (0.5) away â€”
...
outlines fragment the bench into ~27 disconnected navmesh regions (Â§2e), so for
...
it â€” `_corner_slide()` off the feelers, then `_unstick()` â€” are what keep it from
...
state â€” `chase_arrive_distance` (0.5) chasing, `patrol_arrive_distance` (2.0)
patrolling â€” called on every aggro flip **and** from `_apply_phase()`, which sets
...
`_line_of_sight_fallback()` â†’ `_corner_slide()` â†’ `_unstick()`. The order is
...
- **Feelers.** `FeelerLeft` / `FeelerRight` are RayCast3Ds at knee height (y 0.4),
  splayed out from Â±0.3 to Â±0.8 over 1.0 forward, mask 1. Navmesh paths cut
  ...
  reaches it. Both take `add_exception(self)` in `_ready()` â€” they start inside
...
- **The feelers are `enabled = false` and force-updated on demand**, and
  ...
  reactive â€” the enemy grazes the prop first and only slides once that contact
...
- **`_corner_slide()`** deflects by `global_transform.basis.x` (the enemy's right)
  scaled by `CORNER_SLIDE_STRENGTH` (0.75), then renormalises to speed â€” so it is
  ...
  there just picks which wall to grind against â€” that case belongs to `_unstick`.
  Measured: straight-ahead (0, 0, âˆ’8) with only the left feeler blocked becomes
  (4.8, 0, âˆ’6.4), a ~37Â° bend at unchanged speed.
- **`_unstick()`** throws a random lateral dodge once `_stuck_time` passes
  `UNSTICK_STUCK_TIME` (0.6 s), and forces `_since_repath = 999.0` â€” the route it
...
silently kills `_advance_patrol()`'s waypoint give-up â€” the only thing that
rescues a route valid on the navmesh but impassable to the capsule â€” and would
...
members. That may read as natural crowd flow or as jitter â€” give the feelers a
...
threshold for props). It sits **above the phase gate deliberately** â€” an enemy
...
the enemy type. **A fresh hit replaces the current stasis rather than stacking** â€”
...
- Tunables: `move_speed` (8.0), `chase_speed_scale` (1.35), `turn_speed`,
  ...
  `aggro_range` (250.0 â€” fallback only; `_ready()` reads the DetectionArea
  sphere), `eye_height`, `target_height`, `aggro_memory`,
  `chase_repath_interval`, `chase_arrive_distance` (0.5 â€” doubles as the
  direct-pursuit threshold), `patrol_arrive_distance`.

**The DetectionArea is on collision layer 2, and that is the single most
important performance fact about the enemy.** It was mask 1 â€” the layer all ~500
props sit on â€” so every enemy's 250-unit sphere maintained overlap pairs with the
...
At 120 enemies it was worse still â€” 74.8 ms (13 fps), with Jolt logging *"manifold
...
It works because `Player.tscn`'s root now has `collision_layer = 3` â€” layers 1
...
cannot flicker from the staleness â€” the worst case is noticing the player about 7
frames late.
...
making enemies faster â€” a feel change, so it is left as a decision.
...
it switches to CHASE, retargets onto the player and closes 20.6 â†’ 2.4 units in
...
â€” the "out of range" cases no longer exist at r 250, where only cover denies
aggro.
...
1000 HP target that fights back at range, so the fight is dodging and marksmanship
rather than layout.

**`scenes/BossMage.tscn`** â€” `CharacterBody3D`, capsule r 1.0 Ã— h 4.5 centred at
...
idle/run.
- **The scale is 1.711, not the minions' 0.831 Ã— 2.5.** `Skeleton_Mage` is
  **2.630** units tall natively where `Skeleton_Minion` is 2.166 â€” the two
  ...
  4.5 / 2.630 = 1.711 gives exactly 4.5, which *is* 2.5Ã— a 1.8-unit minion.
  Verified as spanning 0.000 â€¦ 4.500 against a capsule spanning the same.
- **`Skeleton_Staff.obj` hangs off a `BoneAttachment3D` on bone `handslot.r`**
  ...
  - **The bone names are lowercase and `handslot.*` is the one you want.** The rig
    ...
    `hand.R` â€” capitalised names do not resolve and the attachment silently sits
    at the model origin.
  - Measured: the attachment resolves to local (1.51, 1.80, 0) â€” out at the hand,
    not at the origin â€” and the staff comes out 3.6 units long against the
    4.5-unit mage.
- **Health 1000** (EASY takes 70% of it), `take_damage(amount)`, signals `health_changed(current, max)`
  and `died`. It joins group **`boss`**.
- **Spell attack** every `spell_interval` (2.6 s): a `spell_2_orb.tscn` rig tinted
  crimson (see Â§2i) travels at `spell_speed` (22), leaving a tinted
  `spell_2_impact.tscn` burst at the point of contact â€” which is also the splash
  ...
  point**, not a second physics query â€” the only thing splash should hurt is the
  ...
  Both attacks detonate audibly â€” see "Music & Sound Effects" for why
  `explosion.mp3` sits at the impact sites rather than in `_shake_player()`.
- **Crushing leap** every `leap_interval` (9 s): a ballistic arc onto the player's
  ...
  out of it. **The arc is committed at launch** â€” the boss does not steer mid-air,
...
**`GameState.start_boss_fight()` runs the entrance in two halves.** First it
clears the `draggable` and `enemies` groups and enters ACTION, then calls
`Player.begin_boss_fight()` â€” staff, both bars, and `scene 2.mp3` â€” and
**`await`s that line finishing** before spawning the Archmage at `BOSS_SPAWN`
(50.5, 73.6, âˆ’4.5) with a `spell_2_impact` burst at his feet.
`Player.attach_boss()` then points the health bar at him. Props go because the
duel needs open space: 500 pieces of clutter would make the leap unreadable and
detonate every orb on scenery.
- **Why deferred:** spawning him immediately had him casting over his own
  introduction. Now the arrival lands on the end of the line.
- **The await is guarded twice.** `voice.playing` is checked first â€” awaiting a
  signal that can never fire hangs forever, so a missing or silent VoicePlayer
  falls straight through to the spawn instead of leaving the fight permanently
  un-started. And `boss_fight` is re-checked after the await, because
  `reset_round()` can clear it while the line plays; without that, resetting mid-
  narration would still drop an Archmage on the bench a few seconds later.
- **`begin_boss_fight()` takes no argument** â€” the boss does not exist yet. It
  raises the bar to the value authored in `Player.tscn` (1000); `attach_boss()`
  corrects it from the real boss, which is what makes EASY's reduced total show.
- **The music swap belongs to `attach_boss()` for the same reason the spawn does.**
  `begin_boss_fight()` runs while `scene 2.mp3` is still playing, so starting
  `BGM_BOSS` there would put battle music under the introduction. See
  "Music & Sound Effects".
- `GameState.boss_fight` gates `WaveSpawner._apply_phase()`. **Without that guard
  ...
  additionally listens to `boss_fight_started` â€” see Â§2g.
- `reset_round()` frees the boss and clears the flag, so dying drops the player on
  ...
  duel â€” a silent respawn loop would be harder to leave than to enter.
- **`_spawn_host()`** picks `current_scene`, falling back to the player's parent.
  ...
  SceneTree, and `current_scene.add_child()` then crashes â€” which is how every
...
- **The F1 (`boss_debug`) debug shortcut is gone**, along with the
  ...
  machinery that existed only to serve it â€” including the
...
1. **Every one of them carries its own script**, and several of their emitters
   ...
   layers, so the scenes are instantiated **whole** â€” harvesting their particle
...
2. **`attack_projectile.tscn` is a `CharacterBody3D` with a collision shape**, not
   ...
   layer 1, so it is inert here â€” but `Bolt._ready()` zeroes its layer and mask
...
3. **They do not clean up after themselves.** Impact bursts get a fuse:
   ...
   that clips `attack_impact`'s 2.0 s Embers tail â€” a burst left standing is a
...
the uniform in place would turn every `spell_2_orb` in the game crimson â€”
...
- Only the emitters exposing an `initial_color` uniform are reached (Glow, Sparks,
  ...
  what carry the read.
- Camera shake on a boss impact is not wired separately â€” it arrives through
...
- **`EASY`**:
  ...
  - `BossMage`: `max_health` × 0.7 (700 against the 1000 baseline). Expressed as a
    multiplier, not an absolute, so raising the boss's health scales both tiers.
- **`MEDIUM`** (Baseline):
  - Standard gameplay tuning: wave 1 size 5, max live 35, 45s interval; enemy speed 8.0, chase 1.35; cannon cooldown 25.0s, slow duration 5.0s; boss health 1000.0.
- **`HARD` (Pitch-Dark Flashlight Mode)**:
  - Combat parameters match MEDIUM baseline.
  - Music: its own tabletop track (`BGM_HARD` / `hard.mp3`) where EASY and MEDIUM
    share `BGM_EZDIF`. See "Music & Sound Effects".
...
off `GameState` â€” no node paths into the round logic.
...
long round cannot melt the frame budget â€” measured 5.83 ms/frame at 35 enemies against a
16.67 ms budget, leaving roughly 3Ã— headroom for rendering and everything else. 120 now
...
**Phase integration.** Only `phase_changed` is connected, **not** `round_reset` â€”
...
`area_path` â€” which defaults to `Navigation/NavSurface`, the tabletop. It is deliberately
**not** `Floor`: that is a 292 Ã— 224 slab at y = âˆ’0.5, ground geometry ~74 units *below*
...
if the snap moved it more than `navmesh_tolerance` (2.5) â€” i.e. it landed inside a prop
...
**The spawner joins group `wave_spawner`** (its `spawner_group` export) so anything
that needs to shut the swarm down finds it without a node path into `Main.tscn`.
`stop_waves()` is that public shutdown — timer stopped, `_wave` rewound, board
cleared — and is what `Player._end_the_swarm()` calls when the Archmage dies. It is
deliberately separate from the PREPARATION branch of `_apply_phase()`: that one is
a reset between attempts, this one means the round is over and no further wave may
arrive whatever the phase does next.
...
**`Coin.tscn`** â€” an `Area3D` (layer 0 / mask 1, so it senses the player and can never
push them) wrapping `coin_gold.gltf` at 1.2Ã—, stood on edge and spinning. Children:
...
Walking over a coin does nothing â€” collection is deliberate. `Coin.gd` reads the key in
...
The navmesh check alone is not enough â€” carve outlines are 2D footprints, so a spot can
...
`sample_spacing` matters a lot â€” at 7.0 only 173 candidates survive and the nearest coin
...
**`all_coins_collected` is the trigger for the Archmage duel** â€” `_ready()` connects it
...
- `_apply_phase()` checks `GameState.boss_fight` **before** the ACTION branch and returns
  ...
  `_apply_phase` once navigation is ready â€” including mid-duel, when F1 was pressed from
  the menu.
...
`clear_coins()` takes no arguments â€” a direct connection does not fail when connected,
...
`tools/BulkPropSetup.gd` (an `EditorScript`, run with **File â†’ Run** in the
...
â”œâ”€â”€ Model                the original node, keeps the authored scale
â”œâ”€â”€ CollisionShape3D     ConvexPolygonShape3D hull, one per mesh
â”œâ”€â”€ CollisionShape3D2    (multi-part props get a compound collider)
â””â”€â”€ NavigationObstacle3D footprint-rectangle carve outline + height
```

- **Colliders are convex hulls, not boxes.** An axis-aligned box around
  something irregular â€” a christmas tree, a rack, a pile of bars â€” is a huge
  invisible barrier blocking the player and the enemy metres from anything they
  can see. Measured against the old boxes, a hull reaches **30â€“37% closer** on a
...
- **Each mesh is MULTI-CONVEX decomposed (V-HACD), not reduced to one hull.** A
  ...
  becomes a solid block from floor to tabletop and the space underneath â€” which
  the player should be able to get into and an enemy should path through â€” stops
...
- **Both decomposition settings matter and the defaults are useless.**
  ...
  fully permissive â€” the decomposer accepts one hull for any shape and the result
  is bit-identical to `create_convex_shape()`. Measured on a synthetic table
  (slab + 4 legs): 1.0 â†’ 1 hull; 0.1 â†’ 4; **0.01 â†’ 6** (slab and legs resolved,
  hollow preserved); 0.001 â†’ 6 (no gain, just slower). `DECOMPOSE_MAX_CONCAVITY`
  is 0.01. `max_convex_hulls` measured identical at 8 and 32, so
  `DECOMPOSE_MAX_HULLS` is 8 â€” the cheap end of the plateau.
- **`Mesh.convex_decompose()` is not exposed to GDScript in Godot 4.7.** The only
  ...
  therefore runs it against a throwaway clone and harvests the shapes â€” calling
...
- Decomposition returns points in **mesh** space, exactly as
  ...
  the authored scale, so every point is transformed into body space â€” skip that
...
- **Cost, measured on the real bench, in both directions:**
  - *Conversion* is slow â€” ~200 ms per prop, ~110 s for all 521. That is a
    one-off tool cost, not a runtime one.
  - *Runtime physics did **not** regress*, despite 3.17Ã— the shapes: headless
    physics time went **3.12 â†’ 2.80 ms/frame mean** and got far more stable
    (p95 4.45 â†’ 2.91, max 5.66 â†’ 2.91). The props are frozen and asleep, so the
    extra hulls cost broadphase and memory rather than narrowphase â€” and the
...
- **`tools/rebuild_prop_colliders.gd`** re-runs the shape builder over props that
  ...
  `--save` to write. It refuses to save if the scene lost a node (see Â§5).
- **The navmesh carve outline is the prop's footprint RECTANGLE, in body space** â€”
  ...
  - *Circle* (`max(size.x, size.z) * 0.5`, the original) is fine on a crate but
    ...
    and 17 long, so it carved a **16.6-unit-wide disc** â€” a ring of invisible
  ...
  - *Octagon inscribed in the footprint* fixes the over-carve (216/252 reachable)
    but cuts the corners, leaving walkable slivers **inside** solid props â€” the
  ...
  rotation â€” that cleared all 522 editor warnings. With the frame square to the
  ...
  **Measured consequence, worth knowing:** with outlines finally accurate, the
  ...
  enemies patrol normally â€” measured 4 of 5 moving, mean 17.2 units in 6 s â€” but
  there is no route across the whole bench, and no navigation setting changes that
  (`agent_radius` 1.5 â†’ 0.4 gives an identical 11/252). It is a level-content limit:
  522 props is more clutter than the table has room for.

- Candidates are chosen **by type, not name** â€” anything already a
...
- `mass = volume * DENSITY` clamped to [0.5, 60], with DENSITY calibrated so the
  ...
  clamp â€” that is deliberate, it keeps the solver sane.
...
- **Freeze.** 500 free rigid bodies resting on the bench never actually sleep â€”
...
- **Obstacle avoidance off.** Every `NavigationObstacle3D` with
  ...
  routes the enemy, so avoidance is only needed for the single prop mid-drag â€”
...
way while dragged â€” a tighter hull is also a narrower one, so they tip more
...
*(Scripts are live; no obstacle nodes are in `Main.tscn` at the moment â€” the
drag mechanic is being prototyped through `PrepCamera.gd` on the bowl instead.)*

**`scenes/Obstacle.gd`** (`class_name Obstacle extends RigidBody3D`) â€” a piece
...
- `freeze = true` + `freeze_mode = FREEZE_MODE_KINEMATIC`, forced in `_ready()`.
  ...
  writing the transform every frame legal â€” a statically frozen body ignores it.
...
- `set_highlighted(on)` tints via emission on a **per-instance duplicate** of
  the shared material â€” highlighting the shared one lights up all nine.
- `grab()` zeroes `collision_layer`/`collision_mask` (deferred â€” collision state
...
**`scenes/BuildMode.gd`** â€” a `Node3D` child of Player that owns the drag tool.
...
- Camera reached by `get_node("../Head/Camera3D")`, **not** an exported
  NodePath â€” see the `node_paths` gotcha in Â§5.
...
- Aiming at a **vertical** face (normalÂ·UP < 0.7) drops the piece to the surface
  underneath the aim point instead of leaving it clinging halfway up the wall.
- Held pieces are forced upright and only yaw â€” a tipped wall is a ramp, and
  enemies would walk over the maze.
- Tunables: `build_reach` (45 â€” the bench is enormous at miniature scale),
  `rotate_step_deg` (15), `follow_speed` (18), `grab_debounce` (0.12).

### Physics Props

**`scenes/Prop.gd`** (`class_name Prop extends RigidBody3D`) â€” the shared script
...
- `apply_hit(point, direction, force)` â€” knocks the prop; the lever arm is
  clamped to `hit_torque_arm` (0.4 m) so a whack on a table corner can't apply
  metres of leverage and spin it through the floor.
- `take_damage(amount)` â€” exists only so `Sword.gd`'s `has_method` probe routes
  hits here; there is no health system yet.
- `carried_by` â€” set by the player. Zeroes `gravity_scale` and `can_sleep`
  while held (a sleeping body ignores the carry spring's velocity writes) and
  restores both on release.

**`scenes/meja.tscn`** â€” a KayKit kitchen table (3.0 Ã— 1.0 Ã— 2.0 m) as a
`RigidBody3D` + `Prop.gd`. Mass 25, friction 0.6, `continuous_cd` on. Collision
is five hand-fitted boxes measured off the mesh: a 3 Ã— 0.2 Ã— 2 tabletop at
y = 0.9, and four 0.19 m legs at (Â±1.35, 0.4, Â±0.85). The model child must
**not** have `top_level = true` (the FBX instance ships with it) â€” that makes
the visual ignore the body's transform, so the collision moves and the table
appears to stay put.

**Player-side prop handling** (`Player.gd`):
- `_push_bodies(approach, delta)` â€” runs after `move_and_slide()`, shoving any
...
- `pick_up_prop()` / `drop_prop()` / `throw_prop()` and `_update_carry(delta)` â€”
...
| Action | Input | Notes |
|---|---|---|
| `lock_in` | F | Preparation â†’ Action. Locks the layout in. **While dead: retry** |
| `restart_round` | G | Action â†’ Preparation. Keeps the layout, resets the player and the enemy. **While dead: retry** (the press is consumed so the reset runs once) |
| `toggle_action_phase` | Enter | Flips whichever phase is current â€” test shortcut |
...
| `reload` | R | **Unused** â€” still defined in `project.godot`, but the Stasis Cannon has no reload |
...
considering the task complete.** Keep entries factual and current â€” prefer
...
- Timestamp every entry `YYYY-MM-DD HH:MM` in **local time**. Get it from the
  system (`Get-Date -Format "yyyy-MM-dd HH:mm"`) â€” never estimate it, and never
  reuse the timestamp of an earlier entry.
- One entry per task, not per file edit. Entry format:

```markdown
## YYYY-MM-DD HH:MM â€” <short title of the task>

**Changed:**
- `path/to/file.gd` â€” what changed and why, in one line.
...
- Record what was actually done, including work that failed or was reverted.
  A recap that only lists successes is worse than no recap â€” the next agent will
  redo the dead end.
- Never rewrite or delete existing entries; the log is append-only.

## 5. Known Gotchas (learned the hard way â€” see project memory for more)
...
- **Hand-edited `.tscn` node references silently break.** Any `[node]` that
  ...
  `[node]` line, or Godot stores a raw `NodePath` that never resolves â€” the
...
- **Editing `project.godot` (autoloads, input actions) while the editor has
  the project open does not take effect until you Project â†’ Reload Current
...
- **A per-frame impulse smaller than static friction moves a rigid body by
  ...
  then lurches when you back off, raise `push_force` â€” don't chase it as a
  collision bug.
- **A `cross(UP)` restoring torque parks an object upside down.** The classic
  ...
  has length `sin(lean)` â€” which is zero at 180Â° as well as at 0Â°. A body that
...
- **`Mesh.create_convex_shape()` returns points in MESH space.** If the physics
  ...
  transformed into the body's space or the collider comes out at 1/scale â€” a
...
- **`Shape3D.get_debug_mesh()` builds a whole mesh just to answer a question
  ...
  fine once â€” but it is not a getter, and calling it per shape per prop costs
...
- **An `Area3D` whose mask includes the layer your scenery is on is O(agents Ã—
  scenery), and it is easy to miss because nothing errors.** Each enemy's
  `DetectionArea` is a 250-unit sphere that was masking layer 1 â€” the same layer
  ~500 props sit on â€” so the broadphase maintained a pair for every enemy against
  every prop. Measured 79% of the entire physics frame at 35 enemies (30.46 â†’
...
- **An enabled `RayCast3D` casts every physics frame whether or not anything
  reads it.** Gating the `is_colliding()` call saves nothing at all â€” the cost is
...
- **`NavigationObstacle3D.avoidance_enabled` is on by default and is not free.**
  ...
  16.7 ms budget â€” more than everything else in the scene put together. Leave it
...
- **Hundreds of resting `RigidBody3D`s never go to sleep.** 500 props settled on
  ...
  and thaw individually â€” do not rely on sleep to save you.
- **`global_transform` on a node that was instantiated but never added to the
  ...
  `SceneTree._initialize()` is also not yet inside it â€” `await process_frame`
...
- **Instantiating `Main.tscn` in a tool and letting one frame process lets the
  ...
  adopts any hand-placed enemy into `enemies` and `_physics_process` â†’
  ...
  missing** â€” 526 instances become 525 and nothing errors. Do the work and pack
...
- **`NavigationObstacle3D` carves the navmesh with `vertices`, not `radius`.**
  ...
  no hole in it â€” no error, no warning, and the agent happily paths straight
...
- **`NavigationObstacle3D` only supports rotation around Y, and it inherits its
  ...
  raised a configuration warning â€” even a 0.3Â° tilt trips it, and one prop was at 175Â°.
...
- **A navmesh carve outline that is not the prop's real footprint is an invisible
  ...
  `NavigationServer3D.map_get_path()` â€” if the physical fill connects but the
...
- **A navigation map query before its first synchronisation fails outright.** It does not
  ...
  synchronization" and hands back a default â€” so `map_get_closest_point()` answers
...
- **A `queue_free()`d node stays in its groups until the end of the frame.** Anything that
  ...
  **both** `round_reset` and `phase_changed` â€” connecting a clear-out to both runs it twice.
...
- **`target_desired_distance` is a hard stop, not a tolerance.** The agent
  ...
  touch them â€” it looks like the enemy losing its nerve, not like a tuning
...
- **`NavigationAgent3D.radius` is the RVO footprint and is independent of the
  collision shape.** Setting it larger than the body â€” 1.2 against a 0.4 capsule
  here â€” makes avoidance treat each agent as three times its real width, so a
...
- **In a hand-written `.tscn`, the 12-number `Transform3D(...)` sets basis
  ROWS, not columns.** Writing the columns gives you the transpose â€” for a
...
- **`AudioStreamPlayer.max_polyphony` does not let one player mix DIFFERENT
  sounds.** `set_stream()` calls `stop()`, so assigning a new stream kills whatever
  was playing; `max_polyphony` only overlaps repeats of the *same* stream. A shared
  SFX player built that way silently truncates every sound the moment another kind
  of sound fires â€” no error, and it looks like a volume or import problem. Give the
  player an `AudioStreamPolyphonic` stream and push each effect through
  `get_stream_playback().play_stream()`.
- **Imported mp3s do not loop.** The `.import` files ship `loop=false`, so a music
  track plays through once and the game goes quiet with nothing to indicate why.
  Set `loop = true` on the `AudioStreamMP3` at runtime rather than in the `.import`
  file, which the editor rewrites on re-import.
- **Autoload identifiers do not resolve under `--script`.** A headless probe that
  says `GameSettings.difficulty` fails to *compile* with "Identifier not found",
  even though the same line is fine in a game script. Fetch the singleton with
  `root.get_node_or_null("GameSettings")` and use the raw enum integers.
- **`Input.mouse_mode` is stubbed under `--headless`** (always reads back
  `VISIBLE`) â€” anything gated on `MOUSE_MODE_CAPTURED` (crosshair visibility,
